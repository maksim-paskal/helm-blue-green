...
func deleteHpa(ctx context.Context, config *config.Type, labelSelector string) error {
	const deleteTypeName = "HorizontalPodAutoscalers"

	deleteType := kube().AutoscalingV2().HorizontalPodAutoscalers(config.Namespace)
