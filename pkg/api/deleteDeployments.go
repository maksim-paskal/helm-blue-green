...
func deleteDeployments(ctx context.Context, config *config.Type, labelSelector string) error {
	log.Debugf("deleting deployments namespace=%s with label selector %s", config.Namespace, labelSelector)

	const deleteTypeName = "Deployments"

	deleteType := kube().AppsV1().Deployments(config.Namespace)
