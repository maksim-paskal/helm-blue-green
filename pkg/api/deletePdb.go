...
func deletePdb(ctx context.Context, config *config.Type, labelSelector string) error {
	const deleteTypeName = "PodDisruptionBudgets"

	deleteType := kube().PolicyV1().PodDisruptionBudgets(config.Namespace)
